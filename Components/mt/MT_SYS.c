...
//chaokw
void MT_SysP2PTransfer(uint8 *pBuf);
void MT_SysGroupTransfer(uint8 *pBuf);
void MT_SysBroadCastTransfer(uint8 *pBuf);

static void MT_SysPkgSendMsg( byte taskID, uint8 event, uint8 *pBuf, uint8 len, uint8 offset );
static void MT_SysChannelUpdate( uint8 *pBuf );
static void MT_SysMACReq( uint8 *pBuf );
static void MT_SysNWKInfoReq( uint8 *pBuf );
static void MT_SysActiveCntReq( uint8 *pBuf );
static void MT_SysGetFWVersionReq( uint8 *pBuf );
static void MT_SysFWUpdateReq( uint8 *pBuf );
static void MT_SysMultiwaySetReq( uint8 *pBuf );
static void MT_SysMultiwaySendReq( uint8 *pBuf );
...
//chaokw
    case MT_SYS_P2P_TRANSFER:
      MT_SysP2PTransfer(pBuf);
      break;
    case MT_SYS_GROUP_TRANSFER:
      MT_SysGroupTransfer(pBuf);
      break;
    case MT_SYS_BROADCAST_TRANSFER:
      MT_SysBroadCastTransfer(pBuf);
      break;

    case MT_SYS_CHANNEL_UPDATE:
      MT_SysChannelUpdate( pBuf );
      break;
    ...
    case MT_SYS_ACTIVE_CNT_REQ:
      MT_SysActiveCntReq( pBuf );
      break;

    case MT_SYS_FW_VERSION_REQ:
      MT_SysGetFWVersionReq( pBuf );
      break;

    case MT_SYS_FW_UPDATE_REQ:
      MT_SysFWUpdateReq( pBuf );
      break;

    case MT_SYS_MULTIWAY_SET_REQ:
      MT_SysMultiwaySetReq( pBuf );
      break;

    case MT_SYS_MULTIWAY_SEND_REQ:
      MT_SysMultiwaySendReq( pBuf );
      break;		
    ...
    case NODE_AUTH_MSG_CMD_RESET_NET:
      Node_auth_uart_msg_process(0, 
            pBuf[MT_RPC_POS_CMD0], 
            pBuf[MT_RPC_POS_CMD1], 
            &pBuf[MT_RPC_POS_DAT0],
            pBuf[MT_RPC_POS_LEN]);
      break;

    case MT_SYS_NWKINFO_REQ:
      MT_SysNWKInfoReq( pBuf );
      break;
...
void MT_SysP2PTransfer(uint8 *pBuf)
{
    ...
    MSGpkt = (afIncomingMSGPacket_t *)osal_msg_allocate( sizeof ( afIncomingMSGPacket_t ) +  pBuf[MT_RPC_POS_LEN] + MT_UART_HEAD_LEN); 
    if(MSGpkt == NULL)
    {
        return;
    }	
	
    MSGpkt->hdr.event = ZDO_P2P_REQ;	
    MSGpkt->cmd.DataLength = pBuf[MT_RPC_POS_LEN] + MT_UART_HEAD_LEN ;   //cmd+len,  no 0xfe
    if ( MSGpkt->cmd.DataLength )
    {
        MSGpkt->cmd.Data = (uint8 *)(MSGpkt + 1);
        osal_memcpy(MSGpkt->cmd.Data, &pBuf[MT_RPC_POS_LEN], MSGpkt->cmd.DataLength);  
    }
    else
    {
        MSGpkt->cmd.Data = NULL;
    }	
    osal_msg_send( GenericApp_TaskID, (uint8 *)MSGpkt );
}
...
void MT_SysBroadCastTransfer(uint8 *pBuf)
{
      ...
      MSGpkt->hdr.event = ZDO_BROADCAST_REQ;
      MSGpkt->cmd.DataLength = pBuf[MT_RPC_POS_LEN] + MT_UART_HEAD_LEN; 
      if ( MSGpkt->cmd.DataLength )
      {
	   MSGpkt->cmd.Data = (uint8 *)(MSGpkt + 1);
	   osal_memcpy(MSGpkt->cmd.Data, &pBuf[MT_RPC_POS_LEN], MSGpkt->cmd.DataLength);  
...
void MT_SysGroupTransfer(uint8 *pBuf)  //chaokw
{
    afIncomingMSGPacket_t *MSGpkt;
    MSGpkt = (afIncomingMSGPacket_t *)osal_msg_allocate( sizeof ( afIncomingMSGPacket_t ) +  pBuf[MT_RPC_POS_LEN] + MT_UART_HEAD_LEN); 
    if(MSGpkt == NULL)
    ...
    MSGpkt->hdr.event = ZDO_GROUP_REQ;
    MSGpkt->cmd.DataLength = pBuf[MT_RPC_POS_LEN] + MT_UART_HEAD_LEN;
    if ( MSGpkt->cmd.DataLength )
    {
        MSGpkt->cmd.Data = (uint8 *)(MSGpkt + 1);
...
{
	afIncomingMSGPacket_t *MSGpkt;
	MSGpkt = (afIncomingMSGPacket_t *)osal_msg_allocate( sizeof ( afIncomingMSGPacket_t )  + pBuf[MT_RPC_POS_LEN] );
	if ( MSGpkt == NULL )
	{
		return;
	}
	MSGpkt->hdr.event = ZDO_CHANNEL_UPDATE;
	MSGpkt->cmd.DataLength = pBuf[MT_RPC_POS_LEN];
	if ( MSGpkt->cmd.DataLength )
	{
		MSGpkt->cmd.Data = (uint8 *)(MSGpkt + 1);
		osal_memcpy( MSGpkt->cmd.Data, &pBuf[MT_RPC_POS_DAT0], MSGpkt->cmd.DataLength );
	}
	else
	{
		MSGpkt->cmd.Data = NULL;
...
static void MT_SysActiveCntReq( uint8 *pBuf )
{
	...
	MSGpkt->hdr.event = ZDO_ACTIVE_CNT_REQ;
	MSGpkt->cmd.DataLength = pBuf[MT_RPC_POS_LEN];
	if ( MSGpkt->cmd.DataLength )
	{
		MSGpkt->cmd.Data = (uint8 *)(MSGpkt + 1);
...
static void MT_SysGetFWVersionReq( uint8 *pBuf )
{
	MT_SysPkgSendMsg( GenericApp_TaskID, ZDO_FW_VERSION_REQ, pBuf, 0, MT_RPC_POS_DAT0 );
}

static void MT_SysFWUpdateReq( uint8 *pBuf )
{
	MT_SysPkgSendMsg( GenericApp_TaskID, ZDO_FW_UPDATE_REQ, pBuf, 0, MT_RPC_POS_DAT0 );
}

static void MT_SysPkgSendMsg( byte taskID, uint8 event, uint8 *pBuf, uint8 len, uint8 offset )
{
	afIncomingMSGPacket_t *MSGpkt;
	MSGpkt = (afIncomingMSGPacket_t *)osal_msg_allocate( sizeof ( afIncomingMSGPacket_t ) + len );
	if ( MSGpkt == NULL )
	{
		return;
	}
	MSGpkt->hdr.event = event;
	MSGpkt->cmd.DataLength = len;
	if ( MSGpkt->cmd.DataLength )
	{
		MSGpkt->cmd.Data = (uint8 *)(MSGpkt + 1);
		osal_memcpy( MSGpkt->cmd.Data, &pBuf[offset], MSGpkt->cmd.DataLength );
	}
	else
	{
		MSGpkt->cmd.Data = NULL;
	}
	osal_msg_send( taskID, (uint8 *)MSGpkt );
}
...
static void MT_SysMultiwaySetReq( uint8 *pBuf )
{
	...
	MSGpkt->hdr.event = ZDO_MULTIWAY_SET_REQ;
	MSGpkt->cmd.DataLength = pBuf[MT_RPC_POS_LEN];
	if ( MSGpkt->cmd.DataLength )
	{
		MSGpkt->cmd.Data = (uint8 *)(MSGpkt + 1);
...
static void MT_SysNWKInfoReq( uint8 *pBuf )
{
	...
	MSGpkt->hdr.event = ZDO_NWKINFO_REQ;
	MSGpkt->cmd.DataLength = pBuf[MT_RPC_POS_LEN];
	if ( MSGpkt->cmd.DataLength )
	{
		MSGpkt->cmd.Data = (uint8 *)(MSGpkt + 1);
