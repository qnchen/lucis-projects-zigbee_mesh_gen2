...
#ifdef PANID_INCREASE
static void ZDOChannelScan( void );  //chaokw
#endif
#if defined ( MANAGED_SCAN )
 ...
 * @fn      ZDOChannelScan()
 *
 * @brief   ZDOChannelScan.
 *
 * @param   none
 *
 * @return  none
 */
static void ZDOChannelScan( void )
{
	static uint8 scanTimesPerChannel = 0;
	static uint32 scanChannel = 0;

	if (0 == scanTimesPerChannel++)
	{
		while (0 == (DEFAULT_CHANLIST & scanChannel))
		{
			scanChannel <<= 1;
			if (0 == scanChannel)
			{
			 	// Default channel 0x0B
				scanChannel = 0x00000800;
			}
		}

		zgDefaultChannelList = DEFAULT_CHANLIST & scanChannel;
		// Error channel
		if (0 == zgDefaultChannelList)
		{
			zgDefaultChannelList = DEFAULT_CHANLIST;
		}
	...
	else if (SCAN_TIMES_PER_CHANNEL == scanTimesPerChannel)
	{
		scanTimesPerChannel = 0;
	}
}
#endif
...
void ZDO_StartDevice( byte logicalType, devStartModes_t startMode, byte beaconOrder, byte superframeOrder )
{
  ...
  {
    //chaokw panid
    if ((_NIB.nwkPanId >= (ZDAPP_CONFIG_PAN_ID + PANID_INCREASE_CNT))
      || (zgConfigPANID >= (ZDAPP_CONFIG_PAN_ID + PANID_INCREASE_CNT)))
    {
      _NIB.nwkPanId = ZDAPP_CONFIG_PAN_ID;
  ...
  if ( ZG_BUILD_JOINING_TYPE && (logicalType == NODETYPE_ROUTER || logicalType == NODETYPE_DEVICE) )
  {
    if ( (startMode == MODE_JOIN) || (startMode == MODE_REJOIN) )
    {
      ...
      ret = NLME_NetworkDiscoveryRequest( zgDefaultChannelList, BEACON_ORDER_15_MSEC );  //chaokw
    #if defined ( ZIGBEE_FREQ_AGILITY )
...
#ifdef PANID_INCREASE  //chaokw panid
	if ( ret == ZSuccess )
	{
		ZDOChannelScan();
		if ( zgScanNwkCount )
		{
			zgConfigPANID = _NIB.nwkPanId = zgConfigPANID_Table[(zgScanNwkCount - 1) / ZG_NWK_DISCOVERY_COUNT];
		}
		else
		{
			zgConfigPANID = _NIB.nwkPanId = zgConfigPANID_Table[zgScanNwkCount / ZG_NWK_DISCOVERY_COUNT];
		}
		if ( zgScanNwkCount++ >= PANID_INCREASE_CNT * ZG_NWK_DISCOVERY_COUNT )
		{
			zgScanNwkCount = 0;
		}
	}
 ...
 * @return      none
 */
void ZDO_ProcessDeviceAnnce( zdoIncomingMsg_t *inMsg )  //chaokw
{
