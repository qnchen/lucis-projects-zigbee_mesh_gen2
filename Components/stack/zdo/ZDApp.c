...
#define MAX_DEVICE_UNAUTH_TIMEOUT   10000  // 10 seconds  //chaokw
...
uint8 join_device_extaddr[SADDR_EXT_LEN] = {0};
ZDO_Permit_Join_Device_t *zdo_join_device = NULL;   //chaokw
...
UINT16 ZDApp_event_loop( uint8 task_id, UINT16 events )
{
  ...
  if ( events & ZDO_DEVICE_RESET )
  {
...
#endif
    {
      ...
      SystemResetSoft();    //chaokw 20170413
    }
  }
                                ...
                                uint8 CapabilityFlags, uint8 type)
{
...
#if NODE_AUTH   //chaokw
  ZStatus_t status = ZSuccess;  //chaokw
#endif  
  ...
  if ( status == ZSuccess )
  {
    if (!Authlist_verify_device(ShortAddress, ExtendedAddress, 0, 0))
      return ZFailure;
  }
#endif
...
#if  1   //chaokw panid
	//Router need Coordinator for premit join
	#ifndef ZDO_COORDINATOR
		osal_memcpy(join_device_extaddr, ExtendedAddress, SADDR_EXT_LEN);
		tmp_join_device = zdo_join_device;
#if 0  //chaokw ping		
              // timeout to free the heap of join device
		osal_start_timerEx( GenericApp_TaskID,
							GENERICAPP_CLEAR_JOIN_DEVICES_EVT,
							GENERICAPP_CLEAR_JOIN_DEVICE_TIMEOUT);
#endif
		while (tmp_join_device)
		{
			if (sAddrExtCmp(tmp_join_device->permit_device.extaddr, join_device_extaddr))
			{
				permit = TRUE;
				break;
			}
		...
		if (permit == FALSE)
		{
			transfer_join_times = 0;
			osal_start_timerEx( GenericApp_TaskID, GENERICAPP_PERMIT_JOIN_MSG_EVT, 100);
			return ZFailure;
		}
