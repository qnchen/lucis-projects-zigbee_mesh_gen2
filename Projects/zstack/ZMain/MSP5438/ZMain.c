...
static void zmain_ext_addr(void)
{
  ...
  {
    // Attempt to read the extended address from the location in the Info memory page
    // where the programming tools know to reserve it.
    if (!osal_memcmp((uint8 *)HAL_NV_IEEE_ADDR, nullAddr, Z_EXTADDR_LEN))
    {
      (void)osal_memcpy(aExtendedAddress, (uint8 *)HAL_NV_IEEE_ADDR, Z_EXTADDR_LEN);
    }
...
static void zmain_cert_init(void)
{
      ...
      (osal_memcmp(certData, nullData, ZCL_KE_CA_PUBLIC_KEY_LEN)))
  {
    ...
    (void)osal_memcpy(certData, (uint8 *)HAL_NV_CA_PUBLIC_KEY_ADDR, ZCL_KE_CA_PUBLIC_KEY_LEN);
    ...
    if (!osal_memcmp(certData, nullData, ZCL_KE_CA_PUBLIC_KEY_LEN))
    {
      ...
      (void)osal_memcpy(certData, (uint8 *)HAL_NV_IMPLICIT_CERTIFICATE_ADDR,
                                           ZCL_KE_IMPLICIT_CERTIFICATE_LEN);
      (void)osal_nv_write(ZCD_NV_IMPLICIT_CERTIFICATE,0, ZCL_KE_IMPLICIT_CERTIFICATE_LEN, certData);
      (void)osal_memcpy(certData, (uint8 *)HAL_NV_DEVICE_PRIVATE_KEY_ADDR,
